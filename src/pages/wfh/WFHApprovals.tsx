...
import {
  CheckCircle, XCircle, Laptop, ChevronLeft, ChevronRight, X, AlertTriangle, MessageSquare,
} from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">WFH Approvals</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Review and manage employee work from home requests</p>
        </div>
      </div>
        ...
        {requests.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 dark:border-gray-700 py-20 px-4 text-center">
            <div className="rounded-full bg-gray-100 dark:bg-gray-800 p-4 mb-4">
              <Laptop className="h-8 w-8 text-gray-300 dark:text-gray-600" />
            </div>
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">No {tab} WFH requests</p>
            <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">WFH requests will appear here once submitted by employees</p>
          </div>
        ) : (
          requests.map((req) => {
            ...
            return (
              <div key={req._id} className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 sm:p-5 transition-all hover:shadow-md">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
                  <div className="rounded-lg bg-gray-50 dark:bg-gray-800 px-3 py-2">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">WFH Date</p>
                    <p className="mt-0.5 text-sm font-bold text-gray-900 dark:text-white">{fmtDate(req.date)}</p>
                  </div>
                  <div className="rounded-lg bg-gray-50 dark:bg-gray-800 px-3 py-2">
                  ...
                  <div className="rounded-lg bg-gray-50 dark:bg-gray-800 px-3 py-2">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">Applied On</p>
                    <p className="mt-0.5 text-sm font-bold text-gray-900 dark:text-white">{fmtDate(req.createdAt)}</p>
                  </div>
                </div>

                {/* Approved by */}
                {req.approvedBy && (
                  <div className="mt-3 flex items-start gap-2 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-3 py-2.5">
                    <MessageSquare className="mt-0.5 h-4 w-4 shrink-0 text-gray-400" />
                    <p className="text-sm text-gray-600 dark:text-gray-300">
                      {req.status === "approved" ? "Approved" : "Rejected"} by <span className="font-semibold">{(req.approvedBy as any)?.name || "Manager"}</span>
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white dark:bg-gray-900 p-6 shadow-2xl dark:shadow-black/40 border border-gray-200 dark:border-gray-800">
            <div className="mb-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-gray-900 dark:text-white">Reject WFH Request</h2>
