...
import {
  ...
  AlertTriangle, MessageSquare, Sparkles, Inbox,
} from "lucide-react";
  ...
  const handleReject = async () => {
    ...
    try {
      await leaveApi.approve(rejectId, {
        status: "rejected",
        rejectionComment: rejectComment || undefined,
      });
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          {pagination && (
            <div className="rounded-xl bg-white/10 px-4 py-2.5 text-center ring-1 ring-white/15 backdrop-blur-sm">
              ...
              <p className="text-xl font-bold tracking-tight">{pagination.total ?? leaves.length}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-3">
        {leaves.length === 0 ? (
          <div className={`${cardCls} flex flex-col items-center gap-2 py-16 text-center`}>
            ...
            <p className="text-sm font-medium text-gray-600 dark:text-gray-300">No {tab} leave requests</p>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {leave.rejectionComment && (
                        <div className="mt-2 flex items-start gap-2 rounded-lg border border-rose-200/70 bg-rose-50/60 px-3 py-2 dark:border-rose-500/20 dark:bg-rose-500/10">
                          ...
                          <div>
                            <span className={`${labelCls} text-rose-600 dark:text-rose-400`}>Rejection Reason</span>
